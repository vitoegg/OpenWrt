...
patch_hostname_ip() {
    ...
    log "Patching LuCI flash reconnect targets"
    flash_js=$(find ./feeds/luci/modules/luci-mod-system/ -type f -name "flash.js" 2>/dev/null)
    if [ -n "$flash_js" ] && [ -f "$flash_js" ]; then
        sed -i "s/, *'immortalwrt\\.lan'//g" "$flash_js"
        sed -i "s/192\\.168\\.[0-9]*\\.[0-9]*/$DEFAULT_IP/g" "$flash_js"
        assert_file_not_contains "$flash_js" "immortalwrt.lan" "LuCI flash reconnect host"
        log "LuCI flash reconnect targets patched"
    else
        log "ERROR: LuCI flash.js not found"
        exit 1
    fi
