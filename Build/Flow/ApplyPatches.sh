...
patch_cloud_docker() {
    local dockerman_dir="feeds/luci/applications/luci-app-dockerman"
    ...
    section "Docker Patches"

    apply_source_patch "$PATCH_ROOT/Docker/010-dockerman-menu.patch" "Dockerman menu"
    apply_source_patch "$PATCH_ROOT/Docker/020-dockerd-remove-iptables-deps.patch" "dockerd dependency"
    apply_source_patch "$PATCH_ROOT/Docker/030-dockerd-host-mode.patch" "dockerd host mode"

    if grep -R -E "admin/services/dockerman|services/dockerman" "$dockerman_dir" >/dev/null; then
    ...
    if grep -Eq '^[[:space:]]*\+(iptables|iptables-mod-extra|IPV6:ip6tables|IPV6:kmod-ipt-nat6|kmod-ipt-nat|kmod-ipt-physdev)([[:space:]]|\\|$)' "$dockerd_makefile"; then
        log "ERROR: dockerd iptables dependency removal failed"
        exit 1
    fi
