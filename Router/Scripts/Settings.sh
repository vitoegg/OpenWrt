...
uci set dhcp.@host[-1].mac='$SWITCH_MAC'
...
uci set network.@device[-1].name='eth1.$VLAN_ID_2'
uci set network.@device[-1].macaddr='$PPPOE_MAC_2'
uci set network.@device[-1].ipv6='0'
uci commit network

# Dual WAN - PPPOE for WAN&WAN2
