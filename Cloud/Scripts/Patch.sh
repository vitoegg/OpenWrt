...
patch_dockerd_without_iptables_deps() {
    local dockerd_makefile="feeds/packages/utils/dockerd/Makefile"

    section "Docker Dependencies"

    require_file "$dockerd_makefile" "dockerd Makefile"

    log "Removing dockerd iptables dependencies for host-only Cloud firmware"
    perl -0pi -e 's/^[ \t]*\+(?:iptables|iptables-mod-extra|IPV6:ip6tables|IPV6:kmod-ipt-nat6|kmod-ipt-nat|kmod-ipt-physdev)[ \t]*(?:\\)?\n//mg' "$dockerd_makefile"
    ...
    log "dockerd iptables dependencies removed successfully"
}
